...
class EmptyStringAsNullDeserializer<T> : JsonDeserializer<T> {
    override fun deserialize(
        json: JsonElement?,
        typeOfT: Type?,
        context: JsonDeserializationContext?
    ): T? {
        if (json != null && json.isJsonPrimitive && json.asJsonPrimitive.isString && json.asString.isNullOrBlank()) return null
        return try {
